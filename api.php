...
switch ($action) {
    ...
    case 'get_favorites':
        $favFile = $config['favorites_file'];
        $favorites = [];
        if (file_exists($favFile)) {
            $parsed = @parse_ini_file($favFile, true);
            if (is_array($parsed) && !empty($parsed)) {
                $idx = 1;
                foreach ($parsed as $fNode => $info) {
                    $favorites[] = [
                        'id'    => $idx++,
                        'node'  => (string)$fNode,
                        'name'  => $info['name'] ?? "Node {$fNode}",
                        'desc'  => $info['desc'] ?? "AllStarLink Node",
                        'loc'   => $info['location'] ?? "Radio Network",
                        'rx'    => isset($info['rx_pct']) ? (int)$info['rx_pct'] : 0,
                        'lcnt'  => isset($info['link_count']) ? (int)$info['link_count'] : 1,
        ...
        if (empty($favorites)) {
            $favorites = [
                ['id' => 1, 'node' => '27339', 'name' => 'W2ECR East Coast Reflector', 'desc' => 'East Coast HUB1', 'loc' => 'Wilmington, NC, USA', 'rx' => 39, 'lcnt' => 145, 'color' => 'red'],
                ['id' => 2, 'node' => '512511', 'name' => 'GB3CR', 'desc' => '433.150 +1.6MHz', 'loc' => 'North Wales UK', 'rx' => 30, 'lcnt' => 1, 'color' => 'red'],
                ['id' => 3, 'node' => '54318', 'name' => 'GB3EG 430.9125', 'desc' => '430.9125 + 7.6MHz', 'loc' => 'Wigan, Greater Manchester', 'rx' => 6, 'lcnt' => 1, 'color' => 'green'],
                ['id' => 4, 'node' => '50683', 'name' => 'GB3PI', 'desc' => '145.750 MHz', 'loc' => 'Barkway, Hertfordshire, UK', 'rx' => 5, 'lcnt' => 1, 'color' => 'green'],
                ['id' => 5, 'node' => '59864', 'name' => 'S21DBA', 'desc' => '438.750 Faridpur, Bangladesh', 'loc' => 'Faridpur, Bangladesh', 'rx' => 2, 'lcnt' => 9, 'color' => 'green'],
                ['id' => 6, 'node' => '66534', 'name' => 'S21SIM', 'desc' => '430.300 MHz Simplex', 'loc' => 'Portable, BD', 'rx' => 0, 'lcnt' => 0, 'color' => 'green'],
                ['id' => 7, 'node' => '41360', 'name' => 'GB7SJ 433.175', 'desc' => '433.175 +1.6 103.5Hz', 'loc' => 'Northwich Cheshire UK', 'rx' => 12, 'lcnt' => 3, 'color' => 'green'],
                ['id' => 8, 'node' => '42235', 'name' => 'K4JDR', 'desc' => '441.725+ Backbone HUB', 'loc' => 'Raleigh, NC, USA', 'rx' => 7, 'lcnt' => 8, 'color' => 'green'],
                ['id' => 9, 'node' => '28404', 'name' => 'K6RRR', 'desc' => 'San Diego Hangout', 'loc' => 'San Diego, CA, USA', 'rx' => 2, 'lcnt' => 5, 'color' => 'green'],
                ['id' => 10, 'node' => '43763', 'name' => 'K6TZ', 'desc' => 'SBARC Hub', 'loc' => 'Santa Barbara, CA, USA', 'rx' => 0, 'lcnt' => 2, 'color' => 'red'],
                ['id' => 11, 'node' => '2196', 'name' => 'M0JKT', 'desc' => 'FreeSTAR UK HUB 1', 'loc' => 'freestar.network', 'rx' => 9, 'lcnt' => 27, 'color' => 'green'],
                ['id' => 12, 'node' => '51288', 'name' => 'M0HOY', 'desc' => 'HUBNet', 'loc' => 'Manchester, UK', 'rx' => 10, 'lcnt' => 24, 'color' => 'green'],
                ['id' => 13, 'node' => '47243', 'name' => 'KR8T', 'desc' => '443.800+ MHz', 'loc' => 'Grand Rapids, MI, USA', 'rx' => 32, 'lcnt' => 1, 'color' => 'red'],
                ['id' => 14, 'node' => '55553', 'name' => 'Parrot+ Dallas', 'desc' => 'Enhanced Audio Echo Parrot', 'loc' => 'Plano, TX, USA', 'rx' => 8, 'lcnt' => 3, 'color' => 'green']
            ];
        }
    ...
    case 'save_favorites':
        $favList = isset($params['favorites']) && is_array($params['favorites']) ? $params['favorites'] : [];
        if (!empty($favList)) {
            $content = "; AllSee v2.0 - AllStarLink Node Favorites Configuration\n";
            $content .= "; Updated: " . date('Y-m-d H:i:s') . "\n\n";
            foreach ($favList as $fav) {
                if (empty($fav['node'])) continue;
                $fNode = preg_replace('/[^0-9]/', '', (string)$fav['node']);
                $fName = addslashes((string)($fav['name'] ?? "Node {$fNode}"));
                $fDesc = addslashes((string)($fav['desc'] ?? "AllStarLink Node"));
                $fLoc  = addslashes((string)($fav['loc'] ?? "Radio Network"));
                $fColor = $fav['color'] ?? 'green';
                $content .= "[{$fNode}]\nname = {$fName}\ndesc = {$fDesc}\nlocation = {$fLoc}\ncolor = {$fColor}\n\n";
            }
            @file_put_contents($config['favorites_file'], $content);
        }
        sendResponse(true, 'Favorites saved successfully');
        break;
    ...
    case 'get_config':
        ...
        if (file_exists($cfgFile)) {
            $cfgData = json_decode((string)@file_get_contents($cfgFile), true) ?: [];
        }
        sendResponse(true, 'Config retrieved', $cfgData);
        break;
    ...
    case 'save_config':
        ...
        $existing = [];
        if (file_exists($cfgFile)) {
            $existing = json_decode((string)@file_get_contents($cfgFile), true) ?: [];
        }
        $updated = array_merge($existing, $params);
        unset($updated['action']);
        @file_put_contents($cfgFile, json_encode($updated, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));
        sendResponse(true, 'Config saved successfully', $updated);
        break;
